...
public class PickUps : MonoBehaviour {
	...
	void Update ()
    {
        if (TrafficGameManager.instance.gameOver == true) return;                          //if gameover is true , then return
        ...
        {                                                                           //check if distance between coin and player is less than 5
            if (Vector3.Distance(transform.position, TrafficGameManager.instance.playerCar.transform.position) < 5f)
                transform.position = Vector3.MoveTowards(transform.position, TrafficGameManager.instance.playerCar.transform.position, 25 * Time.deltaTime); ;    //move the coin towards player
        }
    }

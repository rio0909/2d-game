...
public class CarShop : MonoBehaviour {
    ...
    void SetGameData()
    {
        ...
        TrafficGameManager.instance.normalSpeed        = carDatas[TrafficGameManager.instance.selectedCar].carSpeed;
        TrafficGameManager.instance.turboSpeed         = carDatas[TrafficGameManager.instance.selectedCar].carTurboSpeed;
        TrafficGameManager.instance.fuel               = carDatas[TrafficGameManager.instance.selectedCar].carFuel;
        GuiManager.instance.DistanceMultiplier  = TrafficGameManager.instance.normalSpeed;
    }
    ...
    void SetCarDetails()                                                                        //method which set the car details in the car shop
    {
        ...
        carDatas[currentIndex].carLevel = TrafficGameManager.instance.carDatas[currentIndex].carLevel; //set car level
        if (TrafficGameManager.instance.carDatas[currentIndex].unlocked == true)                       //if selected car is unlocked
        {           
            if (TrafficGameManager.instance.selectedCar == currentIndex)                               //if its selected car
                carShopUI.selectText.text = "Selected";                                         //set Select Btn text to Selected
            else if (TrafficGameManager.instance.selectedCar != currentIndex)                          //if iits not the selected car
                carShopUI.selectText.text = "Select";                                           //set Select Btn text to Select

            if (TrafficGameManager.instance.carDatas[currentIndex].carLevel >= 3)                      //if car level is more than 3
            {
            ...
            else if (TrafficGameManager.instance.carDatas[currentIndex].carLevel < 3)                  //if car level is less than 3
            {   
        ...
        else if (TrafficGameManager.instance.carDatas[currentIndex].unlocked == false)                 //if car is not unlocked
        {
    ...
    public void UpgradeCarBtn()                                                                 //upgrade button method
    {
        ...
        {
            if (TrafficGameManager.instance.coinAmount >= (200 * (carDatas[currentIndex].carLevel + 1)))   //if we have enough coins to upgrade
            {
                TrafficGameManager.instance.coinAmount -= (200 * (carDatas[currentIndex].carLevel + 1));   //reduce the coins
                TrafficGameManager.instance.carDatas[currentIndex].carLevel++;                             //upgrade the level
    ...
    {
        if (currentIndex != TrafficGameManager.instance.selectedCar)                                   //if currentIndex is not equal to selected car
        {
            if (TrafficGameManager.instance.carDatas[currentIndex].unlocked == true)                   //if the car is unlocked
            {
                TrafficGameManager.instance.selectedCar = currentIndex;                                //set the TrafficGameManager.instance.selectedCar to currentIndex    
                TrafficGameManager.instance.Save();                                                    //Save it
                SetCarDetails();                                                                //set car details
                PlayerController.instance.SetCarSprite();                                       //change player car sprite
            }
            else if (TrafficGameManager.instance.carDatas[currentIndex].unlocked == false)             //if the car is not unlocked
            {
                if (TrafficGameManager.instance.coinAmount >= carDatas[currentIndex].carPrice)         //we check if we have enough coins
                {
                    TrafficGameManager.instance.coinAmount -= carDatas[currentIndex].carPrice;         //we reduce the coins
                    TrafficGameManager.instance.carDatas[currentIndex].unlocked = true;                //we unlock it
                    TrafficGameManager.instance.selectedCar = currentIndex;                            //set it as selected car
                    TrafficGameManager.instance.Save();                                                //save it
                    SetCarDetails();                                                            //set car details
                    GuiManager.instance.UpdateTotalCoins();                                     //update the total coins text
                    PlayerController.instance.SetCarSprite();                                   //change player car sprite

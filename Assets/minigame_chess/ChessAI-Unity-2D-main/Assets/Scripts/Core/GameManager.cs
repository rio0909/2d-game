...
namespace Chess.Game {
    public class GameManager : MonoBehaviour {
        ...
        void Update () {
            // 1. Safety check to make sure the board actually exists first
            if (board != null) {
            ...
            if (gameResult == Result.Playing) {
                ...
                // 2. THE LINE 66 FIX: Only update the player if they actually exist!
                if (playerToMove != null) {
                    playerToMove.Update ();
                }

                // 3. Safety check for the clocks
                if (useClocks && whiteClock != null && blackClock != null && board != null) {
        ...
        // --- HIJACKED QUIT BUTTON ---
        public void QuitGame () {
            // Find your MiniGameLoader in the main scene
            MiniGameLoader loader = FindObjectOfType<MiniGameLoader>();
            
            if (loader != null) {
                // Tell it to run your custom close routine!
                loader.CloseMiniGame();
            } else {
                Debug.LogWarning("Could not find the MiniGameLoader to return to Desktop!");
            }
        }
